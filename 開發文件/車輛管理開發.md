...
# 換牌功能 (新增 2025-12-18)
GET    /admin/vehicles/create?source={id}  # 換牌模式新增車輛（複製來源車輛資料）
```
...
#### 3.1 換牌功能頁面 (新增 2025-12-18)
- **路由**: `/admin/vehicles/create?source={vehicleId}`
- **版面**: AdminLayout
- **功能**:
  - 從車輛列表點擊「換牌」按鈕觸發
  - 自動複製來源車輛的所有資料
...
7. **換牌功能** ✅ 新增 (2025-12-18)
   - 快速複製現有車輛資料
   - 自動填充表單欄位（除車牌號碼）
   - 支援民國年日期自動轉換
   - 使用 query parameter 傳遞來源車輛 ID
   - 整合現有權限系統
...
## 💡 換牌功能詳細說明 (2025-12-18 新增)

### 功能概述
換牌功能允許使用者快速複製現有車輛的所有資料，只需重新輸入新車牌號碼，即可新增一輛相同配置但不同車牌的車輛。此功能適用於以下場景：
- 車輛更換牌照但其他資料不變
- 購入相同配置的多輛車輛
- 車輛報廢後重新購入相同規格車輛
...
#### 前端實作
1. **車輛列表頁面 (Index.vue)**
   - 檔案路徑：`resources/js/Pages/Admin/Vehicles/Index.vue`
   - 修改位置：第239-246行
   - 新增綠色「換牌」按鈕於操作欄
   - 按鈕位於「編輯」和「退籍/復籍」之間
   - 使用 `fa-exchange-alt` 圖示
   - 點擊後導航至：`/admin/vehicles/create?source={vehicleId}`

2. **新增車輛頁面 (Create.vue)**
   - 檔案路徑：`resources/js/Pages/Admin/Vehicles/Create.vue`
   - 新增 props：`sourceVehicle` 和 `isReplacementMode`
   - 根據 `isReplacementMode` 動態調整：
     - 頁面標題：「換牌新增車輛」或「新增車輛」
     - 麵包屑導航文字
     - 送出按鈕文字
   - 將 `sourceVehicle` 傳遞給 VehicleForm 組件

3. **車輛表單組件 (VehicleForm.vue)**
   - 檔案路徑：`resources/js/Components/Vehicle/VehicleForm.vue`
   - **無需修改**：已支援透過 `props.vehicle` 接收初始資料
   - 自動填充所有表單欄位（基於 `props.vehicle`）

#### 後端實作
1. **控制器修改**
   - 檔案路徑：`app/Http/Controllers/Admin/VehicleController.php`
   - 修改方法：`create(Request $request)`（第76-139行）
   - 核心邏輯：
     ```php
     // 1. 檢查 source query parameter
...
#### 複製的欄位（共40個）
- **基本資訊**：公司類別、公司、車輛類型、車主名稱、地址
- **製造資訊**：廠牌、出廠年月、車輛形式、排氣量、燃料種類、車輛款式/樣式、引擎號碼、車身號碼、載客人數、顏色、產權類別
- **日期資訊**：發照日期、檢驗日期、入籍日期（自動轉換為民國年）
- **其他資訊**：備註、車隊名稱、車隊類別、車隊編號

#### 不複製的欄位
- **車牌號碼**（`license_number`）：保持空白，使用者必須輸入
- **替補車號**（`replacement_license`）：清空，可重新選擇
- **退籍日期**（`deregistration_year/month/day`）：清空
- **系統欄位**：`id`、`vehicle_status`、`created_by`、`updated_by`、`created_at`、`updated_at`、`deleted_at`

#### 民國年轉換處理
後端使用 `Vehicle::convertWesternToRepublic()` 方法將資料庫的西元年轉換為民國年：
```php
// 發照日期轉換
...
**注意**：`convertWesternToRepublic()` 方法只接受西元年份數字（如 2024），會返回民國年數字（如 113）。

### 權限控制
- **使用權限**：`create vehicles`
- **檢查位置**：
  - 前端：Index.vue 按鈕 `v-if` 檢查
  - 後端：VehicleController 中介軟體保護
- **邏輯**：換牌功能視為「新增車輛」的變體，使用相同權限

### 使用者流程
1. 在車輛列表找到需要換牌的車輛
2. 點擊該車輛的綠色「換牌」按鈕
3. 系統導航到「換牌新增車輛」頁面
4. 表單自動填入來源車輛的所有資料（除車牌號碼）
5. 使用者輸入新車牌號碼
6. 使用者可選擇修改其他欄位
7. 點擊「換牌新增車輛」按鈕送出
8. 系統驗證並新增新車輛
...
### 錯誤處理
- **來源車輛不存在**：頁面以一般新增模式顯示，不報錯
- **來源車輛已軟刪除**：`Vehicle::find()` 不包含軟刪除記錄，返回 `null`
- **權限不足**：前端不顯示按鈕，後端返回 403 錯誤
- **車牌號碼重複**：表單驗證檢查唯一性，顯示錯誤訊息

### 技術優勢
- **實作成本低**：無需新增路由或資料表
- **向後相容**：不影響現有新增車輛功能
- **使用者體驗佳**：一鍵複製，節省重複輸入時間
- **權限控制嚴謹**：重用現有權限系統
- **可維護性高**：程式碼結構清晰，與現有架構一致

### 測試要點
- ✅ PHP 語法檢查通過
- ✅ 前端建置成功（無錯誤）
- ✅ 路由正確註冊
- 🔄 待測試：完整使用流程、民國年轉換正確性、權限控制

### 相關檔案
- `app/Http/Controllers/Admin/VehicleController.php` (第76-139行)
- `resources/js/Pages/Admin/Vehicles/Index.vue` (第239-246行)
- `resources/js/Pages/Admin/Vehicles/Create.vue` (整個檔案)
- `resources/js/Components/Vehicle/VehicleForm.vue` (無需修改)
- `app/Models/Vehicle.php` (使用既有的轉換方法)
